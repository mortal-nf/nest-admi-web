...
const loadTableData = async (params: LoadDataParams) => {
  ...
  const data = await Api.projects.getProjectList(apiParams);
  rowSelection.value.selectedRowKeys = [];
  return data;
};

/**
 * @description 打开操作项目弹窗
 */
const openProjectModal = async (record: Partial<TableListItem> = {}) => {
  ...
  const [formRef] = await showModal({
    modalProps: {
      ...
      onFinish: async (values) => {
        try {
          if (isUpdate) {
            await Api.projects.updateProject(record.id!, values as any);
          } else {
            await Api.projects.createProject(values as any);
          }
  ...
  if (isUpdate && record.id) {
    const detail = await Api.projects.getProjectDetail(record.id);
